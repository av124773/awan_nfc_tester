...
#include <string>
#include <vector>
#include <experimental/filesystem> //_splitpath to implement basename() and dirname
...
std::string Basename(const std::string& path) {
  ...
  std::string fname;//= std::experimental::filesystem::path(&result[0]).filename().string();
  return fname;
}
